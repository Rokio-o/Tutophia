...
class _SessionDetailsScreenState extends State<SessionDetailsScreen> {
  ...
  Future<void> _markComplete() async {
    ...
    setState(() => _busy = true);
    try {
      await BookingRepository.instance.completeBooking(widget.booking.bookingId);
      if (!mounted) return;
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to complete session: $e')),
      );
    } finally {
  ...
  Future<void> _cancelSession() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to cancel session: $e')),
      );
    } finally {
      if (mounted) setState(() => _busy = false);
    }
